...
import { MODULE_TABLE } from "./quotation.filter.js";
...
export const getQuotationDetails = (quotationId) =>
  CommonModel.getMasterDetails(
    ...
    [
      ...
      { type: "LEFT JOIN", table: "leads", alias: "ld", key1: "lead_id", key2: "lead_id" },
    ],
  );

export const getQuotationLines = (quotationId) =>
  CommonModel.getMasterDetails("quotation_items", "*", { quotation_id: quotationId });

export const getNextQuotationId = () =>
  CommonModel.getNextID(MODULE_TABLE, "quotation_id");

export const createQuotationDetails = (data) =>
  CommonModel.saveMasterDetails({ table: MODULE_TABLE, data });

export const updateQuotationDetails = (quotationId, data) =>
  CommonModel.updateMasterDetails({
    table: MODULE_TABLE,
    data,
...
export const createQuotationLine = (data) =>
  CommonModel.saveMasterDetails({ table: "quotation_items", data });

export const createQuotationLines = async (quotationId, lines = []) => {
  const results = [];

  for (const line of lines) {
    results.push(await createQuotationLine({ ...line, quotation_id: quotationId }));
  }

  return results;
};

export const deleteQuotationLines = (quotationId) =>
  CommonModel.deleteMasterDetails({ table: "quotation_items", where: { quotation_id: quotationId } });

export const replaceQuotationLines = async (quotationId, lines = []) => {
...
export const changeCustomersStatus = (ids = []) => CommonModel.changeMasterStatus({ table: MODULE_TABLE, ids: ids, key: "customer_id" });
