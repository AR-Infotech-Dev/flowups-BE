...
ADD COLUMN db_password varchar(255) CHARACTER
SET
    cp1256 COLLATE cp1256_general_ci DEFAULT NULL,
...
ALTER TABLE ab_tickets ADD INDEX idx_tickets_company_base_order (
    company_id,
    amc_call,
    call_direction,
    created_date,
...
ADD INDEX idx_tickets_created_base (
    created_by,
    amc_call,
    call_direction,
    created_date
...
ALTER TABLE ab_ticket_history ADD INDEX idx_history_ticket_new (
    ticket_id,
    field_name,
    action_type,
    new_value (50)
),
ADD INDEX idx_history_ticket_old (
    ticket_id,
    field_name,
    action_type,
    old_value (50)
...
FROM
    ab_admin u
    LEFT JOIN ab_tickets t ON t.assignee = u.adminID
    AND t.status = 'active'
